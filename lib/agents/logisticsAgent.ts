...
import { didArrive, didNotArrive, toldUsNotComing } from "./commitmentRules";
...
export async function calculateDonorETA(
  ...
  try {
    ...
    const history = await db.donorResponseHistory.findMany({ where: { donorId }, select: { status: true, confirmed: true, noShow: true, releasedAt: true, releasedBy: true, releaseReason: true, responseTime: true, notifiedAt: true } });
