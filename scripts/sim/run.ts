 ...
 *
 *   npx tsx scripts/sim/run.ts --n 100000 --seed 42 --out ml/data/sim/v1
 *   npx tsx scripts/sim/run.ts --n 2000 --mix "random:0.5,A:0.1,B:0.1,C:0.1,D:0.05,E:0.05,F:0.05,G:0.05"
...
async function main() {
  ...
  for (let i = 0; i < args.n; i++) {
    ...
    const result = runScenario(spec, { policy: deterministicPolicy, emitRows: !args.quality });
  ...
  if (writer) {
    const manifest = await writer.close({
      ...
      gitSha: gitSha(),
      notes: `Generated by scripts/sim/run.ts with policy=${deterministicPolicy.name}; priors ${PRIORS.version}`,
    });
